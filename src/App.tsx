 ...
 * TypeScript version — works with Vite React TS template
 */
...
type Variation = BorderVariation | DimensionVariation;
...
const MEESHO_DIMS: Record<string, DimConfig> = {
  SQUARE_HD:     { w: 1080, h: 1080, label: "1080×1080", quality: 0.82, note: "Meesho Standard Square" },
  SQUARE_LITE:   { w: 800,  h: 800,  label: "800×800",   quality: 0.72, note: "Lightweight Square" },
  SQUARE_MIN:    { w: 500,  h: 500,  label: "500×500",   quality: 0.65, note: "Min Spec — Smallest File" },
  PORTRAIT_HD:   { w: 900,  h: 1200, label: "900×1200",  quality: 0.78, note: "Portrait — Best for Kurtis/Dresses" },
  PORTRAIT_COMP: { w: 720,  h: 960,  label: "720×960",   quality: 0.68, note: "Compressed Portrait" },
};

// ─── VARIATION CONFIG ─────────────────────────────────────────────────────────

const BORDER_VARIATIONS: BorderVariation[] = [
  { id: "purple-plain",      group: "A", name: "Purple Border",         tag: "Trending",        desc: "Bold purple frame — most popular on Meesho",       borderColor: "#9333ea", badge: null,         cardBg: "#faf5ff", accent: "#9333ea", score: 88, fileSizeNote: "Same as original", recommended: false },
  { id: "yellow-bestseller", group: "A", name: "Yellow + Bestseller",   tag: "🏆 Best Seller",  desc: "Yellow border with Bestseller badge",               borderColor: "#eab308", badge: "bestseller", cardBg: "#fefce8", accent: "#ca8a04", score: 90, fileSizeNote: "Same as original", recommended: false },
  { id: "blue-sale",         group: "A", name: "Blue + SALE Badge",     tag: "⚡ Flash Sale",   desc: "Blue border with SALE sticker — creates urgency",   borderColor: "#2563eb", badge: "sale",       cardBg: "#eff6ff", accent: "#2563eb", score: 87, fileSizeNote: "Same as original", recommended: false },
  { id: "pink-plain",        group: "A", name: "Pink Border",           tag: "Women's Pick",    desc: "Pink frame — best for women's fashion",             borderColor: "#ec4899", badge: null,         cardBg: "#fdf2f8", accent: "#db2777", score: 86, fileSizeNote: "Same as original", recommended: false },
  { id: "green-quality",     group: "A", name: "Green + Quality Badge", tag: "✅ Quality Seal", desc: "Green border + Best Quality badge",                 borderColor: "#16a34a", badge: "quality",    cardBg: "#f0fdf4", accent: "#16a34a", score: 89, fileSizeNote: "Same as original", recommended: false },
];

const DIMENSION_VARIATIONS: DimensionVariation[] = [
  { id: "meesho-standard",    group: "B", name: "Meesho Standard",       tag: "📐 1080×1080px",   desc: "Official Meesho recommended — 1080×1080px JPEG 82%",         dims: MEESHO_DIMS.SQUARE_HD,     cardBg: "#f0f9ff", accent: "#0284c7", score: 94, fileSizeNote: "~120–180 KB", recommended: false },
  { id: "lightweight-square", group: "B", name: "Lightweight Square",    tag: "⚡ 800×800px",      desc: "800×800px at 72% JPEG — 40% smaller than standard",          dims: MEESHO_DIMS.SQUARE_LITE,   cardBg: "#fff7ed", accent: "#ea580c", score: 96, fileSizeNote: "~70–100 KB",  recommended: false },
  { id: "min-spec",           group: "B", name: "Min Spec Compressed",   tag: "🔥 Smallest File",  desc: "500×500px — Meesho minimum, maximum file size reduction",     dims: MEESHO_DIMS.SQUARE_MIN,    cardBg: "#fef2f2", accent: "#dc2626", score: 99, fileSizeNote: "~30–50 KB",   recommended: true  },
  { id: "portrait-hd",        group: "B", name: "Portrait HD",           tag: "👗 900×1200px",     desc: "4:3 portrait — ideal for kurtis, dresses, full-length shots", dims: MEESHO_DIMS.PORTRAIT_HD,   cardBg: "#fdf4ff", accent: "#9333ea", score: 93, fileSizeNote: "~100–150 KB", recommended: false },
  { id: "portrait-compressed",group: "B", name: "Portrait Compressed",   tag: "📦 720×960px",      desc: "Compressed portrait — 68% JPEG, smallest portrait size",      dims: MEESHO_DIMS.PORTRAIT_COMP, cardBg: "#f0fdf4", accent: "#16a34a", score: 97, fileSizeNote: "~50–75 KB",   recommended: false },
];

const ALL_VARIATIONS: Variation[] = [...BORDER_VARIATIONS, ...DIMENSION_VARIATIONS];
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        if (variation.badge === "bestseller") drawBadgeBestseller(ctx, W, H, bw);
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        const maxW = TW * 0.92, maxH = TH * 0.92;
...
async function processVariation(imageSrc: string, variation: Variation): Promise<string> {
  if (variation.group === "A") return processBorderVariation(imageSrc, variation);
...
function CategoryDropdown({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button onClick={() => { setOpen(!open); setCustomMode(false); }} style={{ width: "100%", padding: "11px 16px", borderRadius: 10, border: "1.5px solid #e2e8f0", background: "#fff", color: value ? "#1e293b" : "#94a3b8", fontSize: 14, cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, right: 0, background: "#fff", borderRadius: 12, border: "1.5px solid #e2e8f0", boxShadow: "0 20px 40px rgba(0,0,0,0.12)", zIndex: 1000, maxHeight: 300, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <div style={{ padding: "8px 10px", borderBottom: "1px solid #f1f5f9" }}>
            <input autoFocus placeholder="Search..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ width: "100%", padding: "7px 12px", borderRadius: 8, border: "1.5px solid #e2e8f0", fontSize: 13, fontFamily: "'DM Sans', sans-serif", outline: "none", boxSizing: "border-box" }} />
          </div>
          <div style={{ overflowY: "auto", flex: 1 }}>
            {filtered.map((cat) => (
              <button key={cat} onClick={() => { onChange(cat); setOpen(false); }} style={{ width: "100%", padding: "9px 16px", border: "none", background: cat === value ? "#f0f9ff" : "transparent", color: cat === value ? "#0284c7" : "#374151", fontSize: 13.5, cursor: "pointer", textAlign: "left", fontFamily: "'DM Sans', sans-serif" }}>{cat}</button>
            ))}
            <button onClick={() => { setCustomMode(true); setOpen(false); }} style={{ width: "100%", padding: "10px 16px", border: "none", borderTop: "1px solid #f1f5f9", background: "transparent", color: "#6366f1", fontSize: 13.5, cursor: "pointer", textAlign: "left", fontFamily: "'DM Sans', sans-serif", fontWeight: 600 }}>+ Add Custom Category</button>
          </div>
        </div>
      )}
      {customMode && (
...
function VariationCard({ variation, dataUrl, isLoading, onDownload, originalKB }: {
  ...
  const saving = kb && originalKB ? Math.round(((originalKB - kb) / originalKB) * 100) : null;
  ...
  return (
    <div style={{ borderRadius: 16, border: variation.recommended ? "2px solid #22c55e" : "1.5px solid #e2e8f0", background: "#fff", overflow: "hidden", boxShadow: variation.recommended ? "0 8px 32px rgba(34,197,94,0.15)" : "0 2px 8px rgba(0,0,0,0.06)", transition: "transform 0.2s", position: "relative" }}
      ...
      <div style={{ position: "absolute", top: 10, left: 10, zIndex: 10, background: isGroupB ? "#0f172a" : "#6366f1", color: "#fff", borderRadius: 6, padding: "2px 8px", fontSize: 9, fontWeight: 800, fontFamily: "monospace", letterSpacing: "0.08em" }}>
      ...
      <div style={{ height: 200, background: variation.cardBg, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", position: "relative" }}>
        {isLoading ? (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ color: variation.accent, fontSize: 11, fontFamily: "monospace" }}>Processing...</span>
          </div>
        ) : dataUrl ? (
          <>
            <img src={dataUrl} alt={variation.name} style={{ maxHeight: 190, maxWidth: "100%", objectFit: "contain" }} />
            {kb !== null && <div style={{ position: "absolute", bottom: 8, right: 8, background: saving !== null && saving > 0 ? "#22c55e" : "#64748b", color: "#fff", borderRadius: 6, padding: "3px 8px", fontSize: 10, fontFamily: "monospace", fontWeight: 700 }}>{kb} KB{saving !== null && saving > 0 ? ` ↓${saving}%` : ""}</div>}
          </>
      ...
      <div style={{ padding: "12px 14px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 5 }}>
          ...
          <div style={{ textAlign: "center", background: "#f8fafc", borderRadius: 8, padding: "3px 8px", marginLeft: 8 }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
          {isGroupB && variation.dims && <span style={{ background: "#eff6ff", color: "#2563eb", borderRadius: 5, padding: "2px 7px", fontSize: 10, fontWeight: 700, fontFamily: "monospace" }}>{variation.dims.label}px</span>}
          <span style={{ background: "#f0fdf4", color: "#16a34a", borderRadius: 5, padding: "2px 7px", fontSize: 10, fontWeight: 700, fontFamily: "monospace" }}>{kb !== null ? `${kb} KB` : variation.fileSizeNote}</span>
          {saving !== null && saving > 0 && <span style={{ background: "#fef9c3", color: "#854d0e", borderRadius: 5, padding: "2px 7px", fontSize: 10, fontWeight: 700, fontFamily: "monospace" }}>↓ {saving}% smaller</span>}
        </div>
        ...
        <button onClick={() => dataUrl && onDownload(dataUrl, variation.name)} disabled={!dataUrl || isLoading}
          style={{ width: "100%", padding: "9px", borderRadius: 10, border: "none", background: dataUrl ? variation.recommended ? "linear-gradient(135deg, #22c55e, #16a34a)" : isGroupB ? "linear-gradient(135deg, #0284c7, #2563eb)" : "linear-gradient(135deg, #6366f1, #8b5cf6)" : "#f1f5f9", color: dataUrl ? "#fff" : "#94a3b8", fontSize: 12.5, fontWeight: 700, cursor: dataUrl ? "pointer" : "not-allowed", fontFamily: "'DM Sans', sans-serif" }}>
...
function Sidebar({ activePage, setActivePage, historyCount }: { activePage: string; setActivePage: (p: string) => void; historyCount: number }) {
  ...
  return (
    <aside style={{ width: 220, minWidth: 220, background: "#0f172a", display: "flex", flexDirection: "column", borderRight: "1px solid #1e293b", height: "100vh", position: "sticky", top: 0 }}>
      <div style={{ padding: "24px 20px 18px", borderBottom: "1px solid #1e293b" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ color: "#64748b", fontSize: 10, fontFamily: "monospace" }}>v3.0 PRO</div>
          </div>
        </div>
      </div>
      <nav style={{ flex: 1, padding: "14px 10px" }}>
        <div style={{ marginBottom: 6, padding: "0 8px", color: "#334155", fontSize: 10, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" }}>Tools</div>
        {nav.map((item) => (
          <button key={item.id} onClick={() => setActivePage(item.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 8, border: "none", background: activePage === item.id ? "rgba(99,102,241,0.15)" : "transparent", color: activePage === item.id ? "#a5b4fc" : "#64748b", cursor: "pointer", fontSize: 13.5, fontWeight: activePage === item.id ? 700 : 400, marginBottom: 2, transition: "all 0.15s", textAlign: "left", fontFamily: "'DM Sans', sans-serif", borderLeft: activePage === item.id ? "2px solid #6366f1" : "2px solid transparent" }}>
            <span>{item.icon}</span>
      ...
      <div style={{ padding: "12px 14px", borderTop: "1px solid #1e293b", borderBottom: "1px solid #1e293b" }}>
        <div style={{ fontSize: 9, color: "#475569", fontFamily: "monospace", letterSpacing: "0.08em", marginBottom: 8 }}>VARIATION TYPES</div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}><div style={{ width: 8, height: 8, borderRadius: 2, background: "#6366f1" }} /><span style={{ fontSize: 10, color: "#64748b", fontFamily: "'DM Sans', sans-serif" }}>Border + Badge (5)</span></div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 8, height: 8, borderRadius: 2, background: "#334155" }} /><span style={{ fontSize: 10, color: "#64748b", fontFamily: "'DM Sans', sans-serif" }}>Resize + Compress (5)</span></div>
      </div>
...
export default function ShipOptimize() {
  ...
  const displayedVariations = activeTab === "border" ? BORDER_VARIATIONS : activeTab === "resize" ? DIMENSION_VARIATIONS : ALL_VARIATIONS;
        ...
        <main style={{ flex: 1, overflowY: "auto" }}>
          ...
          <div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "14px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100 }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: "#94a3b8", fontFamily: "monospace" }}>10 variations · Border + Resize + Compress</div>
            </div>
            <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
              {step === 3 && activePage === "optimizer" && (
                <>
                  <button onClick={handleDownloadAll} disabled={!allDone} style={{ padding: "8px 16px", borderRadius: 10, border: "1.5px solid #e2e8f0", background: "#fff", color: "#374151", fontSize: 13, fontWeight: 600, cursor: allDone ? "pointer" : "not-allowed", opacity: allDone ? 1 : 0.5, fontFamily: "'DM Sans', sans-serif" }}>⬇ Download All (10)</button>
            ...
            <div style={{ padding: "24px 32px", animation: "fadeIn 0.3s ease" }}>
              ...
              {step <= 2 && (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 360px", gap: 20, alignItems: "start" }}>
                  <div>
                    ...
                    {previewUrl && <div style={{ marginTop: 10, background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#16a34a", fontFamily: "'DM Sans', sans-serif" }}>✓ Image ready — {originalKB} KB · 10 variations will be generated</div>}
                  </div>
                  <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: "20px", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
                    <div style={{ fontWeight: 700, fontSize: 15, color: "#0f172a", fontFamily: "'DM Sans', sans-serif", marginBottom: 16 }}>Settings</div>
                    <div style={{ marginBottom: 16 }}>
                      <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#374151", fontFamily: "'DM Sans', sans-serif", marginBottom: 8 }}>Product Category</label>
                      <CategoryDropdown value={category} onChange={setCategory} />
                    </div>
                    <div style={{ background: "#f8fafc", borderRadius: 10, padding: "12px", marginBottom: 10 }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: "#6366f1", fontFamily: "monospace", marginBottom: 8, letterSpacing: "0.06em" }}>🎨 GROUP A — BORDER + BADGE (5)</div>
                      {BORDER_VARIATIONS.map((v) => (<div key={v.id} style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 5, fontSize: 11.5, color: "#475569", fontFamily: "'DM Sans', sans-serif" }}><div style={{ width: 12, height: 12, borderRadius: 3, background: v.borderColor, flexShrink: 0 }} /><span>{v.name}</span></div>))}
                    </div>
                    <div style={{ background: "#f0f9ff", borderRadius: 10, padding: "12px", marginBottom: 16 }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: "#0284c7", fontFamily: "monospace", marginBottom: 8, letterSpacing: "0.06em" }}>📐 GROUP B — RESIZE + COMPRESS (5)</div>
                      {DIMENSION_VARIATIONS.map((v) => (<div key={v.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 5, fontSize: 11.5, color: "#475569", fontFamily: "'DM Sans', sans-serif" }}><span>{v.name}</span><span style={{ background: "#dbeafe", color: "#1d4ed8", borderRadius: 4, padding: "1px 6px", fontSize: 10, fontFamily: "monospace" }}>{v.dims.label}</span></div>))}
                    </div>
                    <div style={{ fontSize: 11, color: "#94a3b8", fontFamily: "'DM Sans', sans-serif", marginBottom: 16, lineHeight: 1.5 }}>Group B images are resized to Meesho specs — <strong>smallest file size</strong>, most likely to reduce shipping slab.</div>
                    <button onClick={handleOptimize} disabled={!previewUrl} style={{ width: "100%", padding: "13px", borderRadius: 12, border: "none", background: previewUrl ? "linear-gradient(135deg, #6366f1, #8b5cf6)" : "#e2e8f0", color: previewUrl ? "#fff" : "#94a3b8", fontSize: 15, fontWeight: 700, cursor: previewUrl ? "pointer" : "not-allowed", fontFamily: "'DM Sans', sans-serif", boxShadow: previewUrl ? "0 8px 24px rgba(99,102,241,0.35)" : "none" }}>⚡ Generate 10 Variations</button>
                  </div>
                </div>
              )}
              ...
              {step === 3 && (
                <div style={{ animation: "fadeIn 0.4s ease" }}>
                  {/* Stats bar */}
                  <div style={{ background: "#fff", borderRadius: 16, border: "1.5px solid #e2e8f0", padding: "16px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 24, flexWrap: "wrap", boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 9, color: "#94a3b8", fontFamily: "monospace", marginBottom: 4, letterSpacing: "0.08em" }}>ORIGINAL</div>
                      {previewUrl && <img src={previewUrl} alt="Original" style={{ height: 90, objectFit: "contain", borderRadius: 8, border: "1.5px solid #e2e8f0" }} />}
                    ...
                    <div style={{ flex: 1, minWidth: 100, display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      <div style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)", color: "#fff", borderRadius: 10, padding: "6px 12px", fontSize: 12, fontWeight: 700, fontFamily: "'DM Sans', sans-serif", whiteSpace: "nowrap" }}>⚡ 10 Variations</div>
                      <div style={{ height: 2, flex: 1, background: "linear-gradient(90deg, #22c55e, #e2e8f0)" }} />
                    </div>
                    {[
                      { label: "Original Size", value: `${originalKB} KB`, color: "#ef4444" },
                      { label: "Best Compressed", value: bestKB ? `${bestKB} KB` : "...", color: "#22c55e" },
                      { label: "Max Reduction", value: bestKB && originalKB ? `${Math.round(((originalKB - bestKB) / originalKB) * 100)}%` : "...", color: "#0284c7" },
                  ...
                  {/* Info box */}
                  <div style={{ background: "linear-gradient(135deg, #eff6ff, #f0fdf4)", border: "1.5px solid #bfdbfe", borderRadius: 12, padding: "14px 18px", marginBottom: 20, display: "flex", gap: 14, alignItems: "flex-start" }}>
                    <span style={{ fontSize: 20 }}>💡</span>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 13, color: "#1e40af", fontFamily: "'DM Sans', sans-serif", marginBottom: 4 }}>How These Variations Reduce Shipping Charges</div>
                      <div style={{ fontSize: 12, color: "#374151", fontFamily: "'DM Sans', sans-serif", lineHeight: 1.6 }}>
                        <strong>Group A (Border/Badge):</strong> Creates visually distinct images to differentiate listings.<br />
                        <strong>Group B (Resize/Compress):</strong> Resized to Meesho-standard dimensions at lower JPEG quality — smaller files. The <strong>500×500 Min Spec</strong> is smallest (~30–50 KB). Upload to Meesho and check if your listing shows a reduced shipping slab in your seller dashboard.
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
                    {[{ key: "all", label: "All 10 Variations" }, { key: "border", label: "🎨 Border + Badge (5)" }, { key: "resize", label: "📐 Resize + Compress (5)" }].map((tab) => (
                      <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{ padding: "7px 16px", borderRadius: 20, border: "1.5px solid", borderColor: activeTab === tab.key ? "#6366f1" : "#e2e8f0", background: activeTab === tab.key ? "#6366f1" : "#fff", color: activeTab === tab.key ? "#fff" : "#64748b", fontSize: 12.5, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>{tab.label}</button>
                    ))}
                  </div>
                  ...
                  <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 12, padding: "13px 16px", display: "flex", gap: 10, fontSize: 13, color: "#92400e", fontFamily: "'DM Sans', sans-serif" }}>
                    ...
                    <span><strong>Meesho Seller Tip:</strong> Upload the <strong>500×500 Min Spec</strong> or <strong>720×960 Portrait Compressed</strong> to your listing. These are ~30–75 KB vs your original {originalKB} KB — smallest files most likely to register in a lower shipping weight bracket. Verify in your seller dashboard after uploading.</span>
                  </div>
                </div>
              )}
            </div>
